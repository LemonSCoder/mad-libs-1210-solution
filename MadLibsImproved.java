...
public class MadLibsImproved {
            ...
            try {
                if (i + 6 <= originalStory.length()) {
                    if (originalStory.substring(i, i + 6).equals("<noun>")) {
                        ...
                        finalStory += originalStory.substring(compareStories(originalStory, finalStory, i, k, "Noun"), i) + newNoun;
                    } else if (originalStory.substring(i, i + 6).equals("<verb>")) {
                        System.out.print("Input a verb: ");
                        String newVerb = sc.nextLine();
                        finalStory += originalStory.substring(compareStories(originalStory, finalStory, i, k, "Verb"), i) + newVerb;
                        ...
                        //I'll fix this later.
                        finalStory += originalStory.substring(compareStories(originalStory, finalStory, i, k, "Adjective"), i) + newAdjective;
                    } else if (originalStory.substring(i, i+7).equals("<adverb>")) {
                        System.out.print("Input an adverb: ");
                        String newAdverb = sc.nextLine();
                        //I'll fix this later.
                        finalStory += originalStory.substring(compareStories(originalStory, finalStory, i, k, "Adverb"), i) + newAdverb;
                        ...
                        //I'll fix this later.
                        finalStory += originalStory.substring(compareStories(originalStory, finalStory, i, k, "Plural Noun"), i) + newPluralNoun;
                    }
                }
            } catch (Exception e) {
                ;
    ...
    public static int compareStories(String originalStory, String finalStory, int index, int k, String partOfSpeech) {
        int storyLength = originalStory.substring(k, index).length();
        int returnVal = 0;
        for (int j = storyLength; j < originalStory.length(); j--) {
            if (originalStory.substring(k, index).indexOf(">") == j) {
                returnVal = j + 1;
            }
        }
    ...
    public static int kVal(String originalStory, String finalStory, int index, int k, String partOfSpeech) {
        int storyLength = originalStory.substring(k, index).length();
        int returnVal = 0;
        int addVal = 0;
        ...
        for (int j = storyLength; j < originalStory.length(); j--) {
            if (originalStory.substring(k, index).indexOf(">") == j) {
                k += originalStory.substring(k, index + addVal).length();
            }
        }
        // System.out.println(returnVal);
        return k;
